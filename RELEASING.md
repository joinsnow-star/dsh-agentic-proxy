...
1. 改 `package.json` 的 `version`（改了 `files` 同理——发布内容变了就必须升版本）
2. `npm run verify`（44 项必须全过）
3. `git add -A && git commit && git push`
4. `git tag -a v<version> -m "npm <version>"` — 打在**刚推送的那个提交**上
5. `git push origin v<version>`
6. `npm publish`
7. 等 registry 处理完：轮询 `https://registry.npmjs.org/<name>/<version>` 直到返回 200
   （发布可能是异步的，见下方坑表；`npm view` 在此时会看不到该版本，属正常）
8. `node check-sync.mjs` — 必须输出 `IN SYNC`
...
| npm 发布可能是**异步**的 | registry 返回 `202 Accepted`，日志里写明 `Your package is being processed and may take a few minutes to become available`。此时**版本端点和 tarball 都是 404**，`npm view` 也看不到该版本 | 这不是失败，是排队。0.1.2 从 PUT 到 tarball 可用约 **4 分钟**。判定依据是 `npm publish` 日志里的 `PUT 202`，不是读取结果；轮询 `https://registry.npmjs.org/<name>/<version>` 直到 200 |
| registry CDN 边缘缓存旧 packument | 0.1.1 已发布且 tarball 返回 200，但版本列表仍只有 0.1.0（持续约 95s） | 与上一条是**两回事**：那条是排队，这条是缓存。用 `--prefer-online`，或查版本专属端点（`/<name>/<version>`）绕开 packument 缓存 |
