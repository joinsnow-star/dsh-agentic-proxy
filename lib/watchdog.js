 ...
 * expose a subset, and pinning a node outside it is rejected by the kernel.
 *
...
export class Watchdog {
  ...
  async tick() {
    ...
    try {
      ...
      if (snap.nowState?.state === 'alive') {
        this.untestedSince = 0
        // A pin freezes optimisation; release it once there is a real choice again.
        if (snap.group.fixed !== '' && snap.alive >= 2) {
          await this.api.unpin(this.group)
          this.onEvent(`已有 ${snap.alive} 个可用节点，解除临时指定的「${snap.group.fixed}」`)
        }
        await this.#publish()
        return
      }
  ...
  async #escalate(snap, reason) {
    ...
    try {
      ...
      // (b) Still nothing healthy chosen, so name one explicitly. This pins, which stops
      //     auto-optimising, so it is temporary: tick() releases it once choice returns.
      //     The candidate must be a member — the kernel rejects a pin outside the group.
      const candidate = after.aliveMembers[0]
      if (candidate !== undefined) {
        await this.api.pin(this.group, candidate.name)
        this.onEvent(`重新测速后仍未自动切换，临时指定「${candidate.name}」（${Math.round(candidate.delay)}ms）`)
        await this.#publish()
        return
      }

      // (c) Nothing usable in this group.
      await this.recover(after)
    } catch (error) {
