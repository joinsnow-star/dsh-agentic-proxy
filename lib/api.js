 ...
 *   GET /group/{g}/delay                      ~4s    forces immediate re-test + re-select
 ...
 *   PUT /proxies/{g} {name}                   fast   pins a node (sets `fixed`)
 *   DELETE /proxies/{g}                       fast   releases the pin
 */
...
export class KernelApi {
  ...
  /** Temporarily pin a node. Sets `fixed`, which stops the group from auto-optimising. */
  async pin(group, node) {
    await this.#call('PUT', `/proxies/${encodeURIComponent(group)}`, { name: node })
  }

  /** Release a pin so the group resumes automatic selection. */
  async unpin(group) {
    await this.#call('DELETE', `/proxies/${encodeURIComponent(group)}`)
  }
}
